...
class BaseArgs:
    ...
    # Wan CustomWanPipeline validation / inference (see finetrainers/models/wan/custome.py)
    ...
    # for validation add zgt
    load_lora_weights11: str = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        model_arguments = {
            ...
            "val_num_per_prompt": self.val_num_per_prompt,
            "save_videos_dir": self.save_videos_dir,
            "validation_guidance_scale": self.validation_guidance_scale,
            "validation_flow_guidance_scale": self.validation_flow_guidance_scale,
            "validation_seg_guidance_scale": self.validation_seg_guidance_scale,
            "validation_enable_inner_guidance": self.validation_enable_inner_guidance,
        ...
        training_arguments = {
            ...
            "load_lora_weights11": self.load_lora_weights11,
...
def _add_model_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--val_num_per_prompt", type=int, default=1)
    parser.add_argument("--save_videos_dir", type=str, default=None)
    parser.add_argument(
        "--validation_guidance_scale",
        type=float,
        default=1.8,
        help="Wan validation: text branch scale in CustomWanPipeline (inner or classic CFG).",
    )
    parser.add_argument(
        "--validation_flow_guidance_scale",
        type=float,
        default=0.8,
        help="Wan validation: flow auxiliary scale when inner guidance is enabled.",
    )
    parser.add_argument(
        "--validation_seg_guidance_scale",
        type=float,
        default=0.8,
        help="Wan validation: seg auxiliary scale when inner guidance is enabled.",
    )
    parser.add_argument(
        "--validation_enable_inner_guidance",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Wan validation: use inner-guidance path in CustomWanPipeline (default: on).",
    )
...
def _add_training_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--load_lora_weights11", type=str, default=None)
...
def _map_to_args_type(args: Dict[str, Any]) -> BaseArgs:
    ...
    result_args.val_batch_size = args.val_batch_size
    result_args.val_num_per_prompt = args.val_num_per_prompt
    result_args.save_videos_dir = args.save_videos_dir
    result_args.validation_guidance_scale = args.validation_guidance_scale
    result_args.validation_flow_guidance_scale = args.validation_flow_guidance_scale
    result_args.validation_seg_guidance_scale = args.validation_seg_guidance_scale
    result_args.validation_enable_inner_guidance = args.validation_enable_inner_guidance
    ...
    result_args.load_lora_weights11 = args.load_lora_weights11
